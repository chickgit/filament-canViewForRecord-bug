                ...
                fetchEventsUsing: async ({ start, end, allDay }, successCallback, failureCallback) => {
                    ...
                    return $wire.fetchEvents({ start, end, allDay })
                        .then(events => {
                            ...
                            Object.keys($data.cachedEvents).forEach((cachedEventId) => {
                                if (!events.some((event) => event.id === cachedEventId)) {
                                    delete $data.cachedEvents[cachedEventId];
                                }
                            });

                            {{-- If no events fetched, return cached events --}}
                            if (events.length === 0) {
